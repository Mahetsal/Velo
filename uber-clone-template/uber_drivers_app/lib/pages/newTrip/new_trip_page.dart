...
class _NewTripPageState extends State<NewTripPage> {
  ...
  getLiveLocationUpdatesOfDriver() {
    ...
    positionStreamNewTripPage =
      ...
      http.put(
        Uri.parse("$_awsApiBaseUrl/trips/${widget.newTripDetailsInfo!.tripID!}"),
        headers: {
          "Authorization": "Bearer public-migration-token",
          "Content-Type": "application/json"
        },
        body: jsonEncode({"driverLocation": updatedLocationOfDriver}),
      );
    });
  }
  ...
  endTripNow() async {
    ...
    await http.put(
      ...
      body: jsonEncode({"fareAmount": finalFareAmount, "status": "ended"}),
    );
  ...
  saveFareAmountToDriverTotalEearning(String fareAmount) async {
    ...
    if (getResp.statusCode == 200) {
      ...
      previousTotalEarning =
          double.tryParse(item["earnings"]?.toString() ?? "0") ?? 0.0;
    }
    final fareAmountForThisAmount = double.tryParse(fareAmount) ?? 0.0;
    final newTotalEarning = previousTotalEarning + fareAmountForThisAmount;
    await http.put(
      Uri.parse("$_awsApiBaseUrl/drivers/$driverUid"),
      headers: {
        "Authorization": "Bearer public-migration-token",
      ...
      body: jsonEncode({"earnings": newTotalEarning.toStringAsFixed(2)}),
    );
  }
