...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  Future<bool> _isPhoneUnique(String phoneNumber) async => true;
  ...
  Future<void> signUpFormValidation() async {
    if (userNameTextEditingController.text.trim().length < 3) {
      ...
      final unique = await _isPhoneUnique(userPhoneTextEditingController.text);
      if (!unique) {
        cMethods.displaySnackBar(
  ...
  uploadImageToStorage() async {
    cMethods.displaySnackBar(
      "Legacy email signup is retired. Use phone signup flow.",
      context,
    );
    if (!mounted) return;
