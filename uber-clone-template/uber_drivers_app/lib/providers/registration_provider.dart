...
class RegistrationProvider extends ChangeNotifier {
  ...
  String _subscriptionNextDueDate = "";
  bool get isSubscriptionActive => _isSubscriptionActive;
  String get subscriptionNextDueDate => _subscriptionNextDueDate;
  ...
  Future<bool> _isPhoneUniqueAcrossUsersAndDrivers(String phoneNumber) async {
    final normalized = _normalizeJordanPhone(phoneNumber);
    final usersResponse = await http.get(
      Uri.parse("$_awsApiBaseUrl/users/by-phone/${Uri.encodeComponent(normalized)}"),
    );
    if (usersResponse.statusCode == 200) {
      final usersData = jsonDecode(usersResponse.body) as Map<String, dynamic>;
      if ((usersData["exists"] ?? false) == true) {
        return false;
      }
    }
    ...
    if (driversResponse.statusCode == 200) {
      final driversData =
          jsonDecode(driversResponse.body) as Map<String, dynamic>;
      if ((driversData["exists"] ?? false) == true) {
        return false;
      }
    }
    return true;
  ...
  Future<String> _getPresignedUploadUrl({
    ...
    final response = await http.post(
      Uri.parse("$_awsApiBaseUrl/uploads/presign"),
    ...
    return data["uploadUrl"].toString();
  }
  ...
  Future<String> _uploadFileToS3({
    ...
    final uploadUrl = await _getPresignedUploadUrl(
      filename: filename,
      folder: folder,
      contentType: contentType,
    );
  ...
  Future<String> uploadImageToFirebaseStorage(
    ...
    return _uploadFileToS3(photo: photo, folder: path);
  }

  Future<void> _putDriverData(Map<String, dynamic> data) async {
    final uid = _uid;
    if (uid == null) {
      throw Exception("User is not signed in.");
    }
    final response = await http.put(
      Uri.parse("$_awsApiBaseUrl/drivers/$uid"),
      headers: {"Content-Type": "application/json"},
    ...
    if (response.statusCode == 404) {
      final createResponse = await http.post(
        ...
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({...data, "id": uid}),
      );
      if (createResponse.statusCode < 200 || createResponse.statusCode >= 300) {
        throw Exception("Failed to create driver data.");
      }
      return;
    }
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception("Failed to update driver data.");
    }
  }
  ...
  Future<void> saveUserData(BuildContext context) async {
    ...
    try {
      final phone = _normalizeJordanPhone(phoneController.text.trim());
      if (phone.isEmpty || phone.length < 7) {
        commonMethods.displaySnackBar("A valid phone number is required.", context);
        return;
      }
      final isUnique = await _isPhoneUniqueAcrossUsersAndDrivers(phone);
      if (!isUnique) {
        commonMethods.displaySnackBar(
            "This phone number is already used by another account.", context);
        return;
      }
      ...
      final driver = Driver(
        id: _uid ?? DateTime.now().millisecondsSinceEpoch.toString(),
      ...
      driverMap["acceptedTerms"] = true;
      driverMap["acceptedTermsVersion"] = "1.0";
      driverMap["acceptedTermsAt"] = DateTime.now().toIso8601String();
      driverMap["approvalStatus"] = "pending";
      driverMap["activeStatus"] = "inactive";
      driverMap["monthlySubscription"] = {
        "isActive": false,
        "plan": "monthly",
        "status": "inactive",
        "paymentMethod": "",
      ...
      await _putDriverData(driverMap.cast<String, dynamic>());
      stopLoading();
    } catch (e) {
      stopLoading();
...
// Fetch the driver's earnings from Firebase Database
  Future<void> fetchDriverEarnings() async {
    try {
      ...
      final response =
          await http.get(Uri.parse("$_awsApiBaseUrl/drivers/$userId"));
      if (response.statusCode == 200) {
        final payload = jsonDecode(response.body) as Map<String, dynamic>;
        if ((payload["exists"] ?? false) != true || payload["item"] == null) {
          _driverEarnings = 0.0;
        ...
        double earnings = double.tryParse(item["earnings"].toString()) ?? 0.0;
        final Map<String, dynamic> subscription =
            ((item["monthlySubscription"] ?? {}) as Map)
                .cast<String, dynamic>();
        _isSubscriptionActive = subscription["isActive"] == true;
        _subscriptionNextDueDate =
            subscription["nextDueDate"]?.toString() ?? "";
  ...
  Future<void> retrieveCurrentDriverInfo() async {
    try {
      ...
      final response =
          await http.get(Uri.parse("$_awsApiBaseUrl/drivers/$driverId"));
      if (response.statusCode == 200) {
        final payload = jsonDecode(response.body) as Map<String, dynamic>;
        if ((payload["exists"] ?? false) != true || payload["item"] == null) {
          return;
  ...
  Future<void> updateBasicDriverInfo(BuildContext context) async {
    try {
      ...
      await _putDriverData(driverData);
      _isLoading = false;
      notifyListeners();
    } catch (e) {
  ...
  Future<void> updateVehicleBasicInfo(BuildContext context) async {
    try {
      ...
      await _putDriverData({"vehicleInfo": vehicleData});
      _isLoading = false;
      notifyListeners();
    } catch (e) {
      print("An error occurred while updating basic driver info: $e");
